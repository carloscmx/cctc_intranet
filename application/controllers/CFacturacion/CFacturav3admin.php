...
class CFacturav3admin extends CI_Controller
{
	...
	function catalogoTipoGasto($string)
	{
		$metodo = "";
		switch ($string) {
			case "G01 - Adquisición de Mercancías":
				$metodo = "G01";
				break;
			case "G02 - Devolución descuentos o bonificaciones":
				$metodo = "G02";
				break;
						...
						if ($this->utilerias->valida_rfc($rfccliente)) {
							...
							switch ($facturaresponse->codigo) {
								case "200":
									...
									if ($datoscliente->groupid == 0) {
										$xmlurl = url_cliente_factura_archivo($tokenxml);
										$pdfurl = url_cliente_factura_archivo($tokenpdf);
									} else {
