...
class RouterosAPI
{
    ...
    var $port      = 2090;  //  Port to connect to (default 8729 for ssl)
    ...
    public function connect($ip, $login, $password, $puerto_d)
    {

        /*Se agregan las variables para conexion directa con los accesos asignados de manera predeterminada */
        ...
        for ($ATTEMPT = 1; $ATTEMPT <= $this->attempts; $ATTEMPT++) {
            ...
            $this->debug('Connection attempt #' . $ATTEMPT . ' to ' . $PROTOCOL . $ip . ':' . $puerto_d . '...');
            $this->socket = @stream_socket_client($PROTOCOL . $ip . ':' . $puerto_d, $this->error_no, $this->error_str, $this->timeout, STREAM_CLIENT_CONNECT, $context);
            if ($this->socket) {
